...
#include "State/PlayerCounterState.h"
...
void PlayerStateObserver::SwitchState(const PlayerData::PlayerBehaviorState& _switchState)
{
	...
	else if (_switchState == PlayerData::State_Guard)
	{
		validStateController->SwitchState<PlayerGuardState>();
	}
	else if (_switchState == PlayerData::State_Counter)
	{
		validStateController->SwitchState<PlayerCounterState>();
	}
	// どれでもないなら待機移動ステートに行く
	else
	{
