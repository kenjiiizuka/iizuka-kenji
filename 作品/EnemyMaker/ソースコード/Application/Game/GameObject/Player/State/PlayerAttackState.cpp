...
void PlayerAttackState::Entry()
{
	...
	// 入力にあわせて攻撃を開始 回避からの派生ならYのみ
	if (inputController->IsInput(PlayerInputControllerComponent::Operation_AttackB))
	{
		attack = mAttackData.lock()->GetNextBAttackInformation(PlayerData::AttackID_NoAttack);
	}
	else if (inputController->IsInput(PlayerInputControllerComponent::Operation_AttackY))
	{
...
{
	// 初回フレームは処理しない
	// ステップアニメーションから攻撃アニメーションへ遷移する際に入力が同じフレーム内で入力が二回行われてしまうことを防ぐ
	if (mbEntryFrame)
	{
		mbEntryFrame = false;
		return;
...
void PlayerAttackState::Comb()
{
	...
	if (inputController->IsInput(PlayerInputControllerComponent::Operation_AttackY))
	{
		// 次の攻撃がなければ攻撃不可にする
	...
	else if (inputController->IsInput(PlayerInputControllerComponent::Operation_AttackB))
	{
		// 次の攻撃がなければ攻撃不可にする
