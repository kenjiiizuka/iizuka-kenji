...
void PlaneComponent::Init(Vector3 _size, Vector3 _offset, const std::string& _textureFilePath, const RendererInf::Layer _registerLayer, const bool _isLighting)
{
	struct Vertex
	{
		Vector3 mPosition;
	...
	RendererManager::GetInstance().RegisterEntity(mVisualEntity, _registerLayer);
}
...
void PlaneComponent::SetTexture(const Texture _texture)
{
	mVisualEntity->GetVisualEntityData(0)->SetMaterialTexture(_texture);
}

void PlaneComponent::SetWorldMatrix(const DirectX::SimpleMath::Matrix& _matrix) noexcept
{
