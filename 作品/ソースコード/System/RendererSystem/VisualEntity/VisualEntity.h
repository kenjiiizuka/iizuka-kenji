...
{
	Draw_SolidBack,  /**< ぬりつぶしの背面カリング */
	Draw_SolidFront, /**< ぬりつぶしの前面カリング */
	Draw_WireFrame,  /**< ワイヤーフレームのカリングなし */
};
...
　　 * @fn　SetDrawType
　　 * @brief 描画の種類のセット
     * @param DrawType (_type) 
	 * @param const size_t (_shaderID)
　　 * @return void
　　*/
	inline void SetDrawType(DrawType _type, const size_t _shaderID = 0);
...
inline void VisualEntity::SetDrawType(DrawType _type, const size_t _shaderID)
{
...
inline std::shared_ptr<VShader> VisualEntity::MakeVertexShader()
{
	...
	return std::static_pointer_cast<VShader>(mVertexShader[mVertexShader.size() - 1]);
}
...
inline std::shared_ptr<PShader> VisualEntity::MakePixelShader()
{
	...
	return std::static_pointer_cast<PShader>(mPixelShader[mPixelShader.size() -1]);
}
